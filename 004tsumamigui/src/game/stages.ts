 ...
 * 自動プレイのシミュレーション（各 60 回）で決めた値：
 *   予兆で 0.3 秒後に放し、0.25 秒後に押しなおす上手な人   → 中央値 6,740（9 割近くがクリア）
 *   0.6 秒後に押しなおす人                                  → 中央値 5,970（半分ほどがクリア）
 *   1.2 秒後に押しなおす慎重な人                            → 中央値 4,610（真ん中のデモ）
 *   ほおばりまで押し続けない人（1.2 秒ごとに放す）          → 中央値 3,170（いちばん下のデモ）
 */
export const ENDING_SCORES = [3500, 6000] as const;
